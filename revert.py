...
code = re.sub(r'# Model 5.*?(?=# Find best model)', r'', code, flags=re.DOTALL)

# And restore models list
old_models = """models = [
...
# we need to restore MCMC block
code = re.sub(r'if best_model\["id"\] in \[4, 8\]:.*?else:\n.*?opt_init = best_model\["res"\].x', r"""if best_model["id"] == 4:
    ndim = 6
